...
data_summary table has a list inside, so need to remove it so it's exportable as a csv
```{r}
data_summary_exportable <- data_summary |>
  select(-violations)
```


```{r}
write.csv(data_clean, "processed_data/data_clean.csv", row.names = FALSE)
write.csv(data_summary_exportable, "processed_data/data_summary.csv", row.names = FALSE)
