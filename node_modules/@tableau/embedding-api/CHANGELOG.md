...
- Lazy loading is now enabled by default on the embedded iframe's `loading` attribute.

## [3.14.0] - 2025-08-05

### Added

- Added `launchAnalyticsAssistantAsync` to `TableauViz` and `TableauAuthoringViz` where calling it
  launches the Analytics Assistant (Tableau Agent) side pane
- Added `PulseMetricSizeChanged` event to `TableauPulse`
- Added `IsAnalyticsAssistantAvailable` to `TableauViz` and `TableauAuthoringViz` which can be used
  to determine if Analytics Assistant is available for use
